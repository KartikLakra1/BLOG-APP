  ...
  const getUserBlogs = async () => {
    try {
      ...
      const { data } = await axios.get(
        `https://blog-app-4sc3.onrender.com/api/v1/blog/user-blog/${id}`
      );
        ...
        {total > 0 ? (
          ...
          <h1>
            ...
            {/* <NavLink to={"/userdashboard/createblog"}>
              <h1 className="font-bold cursor-pointer text-xl bg-white text-black p-2 mt-2 md:w-fit w-full hover:bg-slate-300 rounded-md text-center">
                WRITE 
              </h1>
            </NavLink> */}
            <h1 className="font-bold cursor-pointer text-xl bg-white text-black p-2 mt-2 md:w-fit w-full hover:bg-slate-300 rounded-md text-center">
